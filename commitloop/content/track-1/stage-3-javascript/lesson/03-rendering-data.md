...
Log in Console to verify: `console.log(workouts.length)` before and after render.
...
3. Join and assign to an element's `innerHTML`
...
Put list rendering in one function. When data changes — add, delete, edit — call that function again. Inspect `#workout-list` in Elements after each call to confirm the DOM updated.
