...
Client-side validation is for **fast feedback**. The API will enforce rules again in Stage 4 — never trust the browser alone in production.
...
form.reset();
renderWorkouts();
console.log("added, now", workouts.length, "items");
```

The user sees the new item immediately. Check Elements — the new `<li>` should be there.
